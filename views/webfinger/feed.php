...
<feed xml:lang="en-US" xmlns="http://www.w3.org/2005/Atom" 
  ...
  <generator uri="<?= $GLOBALS['ABSOLUTE_URI_STUDIP'] ?>" version="<?= $GLOBALS['SOFTWARE_VERSION'] ?>"><?= studip_utf8encode(htmlspecialchars($GLOBALS['UNI_NAME_CLEAN'])) ?></generator>
  <id><?= htmlspecialchars($GLOBALS['ABSOLUTE_URI_STUDIP']."plugins.php/ostatus/webfinger/feed/".$user['username']) ?></id>
  <title><?= studip_utf8encode(htmlspecialchars($user->getName())) ?> timeline</title>
  <subtitle>Updates from <?= studip_utf8encode(htmlspecialchars($user->getName())) ?> on <?= studip_utf8encode(htmlspecialchars($GLOBALS['UNI_NAME_CLEAN'])) ?>!</subtitle>
  ...
  <link href="<?= htmlspecialchars($GLOBALS['ABSOLUTE_URI_STUDIP']."plugins.php/ostatus/webfinger/feed/".$user['username']) ?>" rel="self" type="application/atom+xml"/>
  <? foreach ($blubber as $blubb) : ?>
  <entry>
    <title><?= studip_utf8encode(htmlspecialchars($blubb['name'])) ?></title>
